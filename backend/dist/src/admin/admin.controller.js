...
let AdminController = class AdminController {
    ...
    activityLogsService;
    usersService;
    constructor(projectsService, activityLogsService, usersService) {
        this.projectsService = projectsService;
        this.activityLogsService = activityLogsService;
        this.usersService = usersService;
    }
    async getGlobalStats(empresaId, rol) {
        const stats = await this.projectsService.getStats(empresaId, rol === user_entity_1.UserRole.SUPER_ADMIN);
        const technical = await this.activityLogsService.getTechnicalMetrics(empresaId);
        const usersCount = await this.usersService.count(empresaId, rol === user_entity_1.UserRole.SUPER_ADMIN);
        return {
            ...stats,
            users: usersCount,
            technical,
